...
describe('الفترات الماضية', () => {
  ...
  it('يحتسب ثلاثاً في كل دفعة، ويدور حتى يبلغها كلَّها ثم يبدأ بأقدمها احتساباً', async () => {
    ...
    // عشر دفعاتٍ من ثلاث تبلغ التسع والعشرين كلَّها
    expect(new Set(computed).size).toBe(29);

    computed.length = 0;
    hour();
    await recomputePastPeriods(env, AT);
    /* والتسع والعشرون لا تنقسم على ثلاث: العاشرة أتمّت آخر فترتين وأعادت
       أقدم ما احتُسب — أولى الدفعة الأولى. فالحادية عشرة تأخذ ما بقي منها
       ثم أولى الثانية: أقدمُها احتساباً، لا ما جاء أوّلاً في القائمة. */
    const key = (p: { kind: string; start: string }) => `${p.kind}:${p.start}`;
    expect(computed).toEqual([key(first[1]), key(first[2]), key(second[0])]);
  });
});
