    ...
    <div>
      ...
      <div className="card" style={{ marginBottom: 16, padding: 14 }}>
        ...
        <div className="row" style={{ marginTop: 12 }}>
          ...
          <div className="spacer" />
          <span className="muted" style={{ fontSize: 'var(--text-sm)' }}>{filtered.length} عنصر</span>
...
function KanbanView({ rows, navigate, onMove }: any) {
  ...
  return (
    <div style={{ overflowX: 'auto' }}>
      <p className="muted" style={{ fontSize: 'var(--text-xs)', marginTop: 0, display: 'flex', alignItems: 'center', gap: 6 }}><Lightbulb size={14} /> اسحب البطاقة إلى العمود التالي لتحريك مرحلتها (ضمن التسلسل المسموح).</p>
      ...
      {items.length > 0 && (
        <div style={{ marginTop: 14 }}>
          ...
          <div style={{ maxHeight: 160, overflow: 'auto', border: '1px solid var(--border)', borderRadius: 'var(--radius)', padding: 8 }}>
            {items.slice(0, 20).map((it, i) => <div key={i} style={{ fontSize: 'var(--text-sm)', padding: '2px 0' }}>• {it.title || '(بدون عنوان)'}</div>)}
            {items.length > 20 && <div className="muted" style={{ fontSize: 'var(--text-xs)' }}>… و{items.length - 20} غيرها</div>}
          </div>
        </div>
      )}
