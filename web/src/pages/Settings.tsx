...
function Permissions() {
  ...
  return (
    <div className="card">
      ...
      <p className="muted" style={{ fontSize: 'var(--text-sm)' }}>التعديل يسري فوراً على كل العمليات (يُتحقق منه على الخادم).</p>
      <table className="table">
          ...
          {KNOWN_PLATFORMS.map((p) => {
            ...
            return (
              ...
              >
                ...
                <span style={{ fontWeight: 600, fontSize: 'var(--text-sm)' }}>{PLATFORM_META[p].label}</span>
                <div className="spacer" />
  ...
  return (
    <div className="card">
      ...
      <div className="card" style={{ background: 'color-mix(in oklab, var(--muted) 40%, transparent)', marginTop: 12, marginBottom: 12 }}>
        ...
        <label className="muted" style={{ fontSize: 'var(--text-sm)', display: 'inline-flex', gap: 6, cursor: 'pointer', marginTop: 6 }}>
    ...
    <div className="card" style={{ background: 'color-mix(in oklab, var(--muted) 40%, transparent)', marginBottom: 12 }}>
      ...
      {h.accounts_error ? (
        ...
        <div style={{ marginBottom: 10 }}>
          ...
          {(h.accounts || []).length === 0 && <p className="muted" style={{ fontSize: 'var(--text-xs)' }}>لا توجد حسابات.</p>}
        </div>
      )}
