...
async function request<T = any>(path: string, options: RequestInit = {}): Promise<T> {
  ...
  // فالجلسة تنتهي أثناء العمل لا قبل بدئه. والحارس يردّ على مسارات `‎/api/`
  ...
  //
  // والفحص على وجود `login` لا على رمز الخطأ: المنصة نفسها تردّ 401 بجملة
  // عربية بلا هذا الحقل، وهاتان حالتان مختلفتان.
  //
